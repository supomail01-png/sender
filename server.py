...
@app.post("/anydesk/register")
async def register_anydesk(info: AnyDeskInfo, _: bool = Depends(verify_user_key)):
    """User ki-register AnyDesk ID dyalo"""
    now = int(time.time())
    with get_db() as db:
        db.execute("""
            UPDATE users SET anydesk_id=?, anydesk_password=?, updated_at=?
            WHERE username=?
        """, (info.anydesk_id, info.anydesk_password, now, info.username))
        # Ila user mafichi f users, insert
        if db.execute("SELECT changes()").fetchone()[0] == 0:
            db.execute("""
                INSERT INTO users (username, anydesk_id, anydesk_password, created_at, updated_at)
                VALUES (?, ?, ?, ?, ?)
            """, (info.username, info.anydesk_id, info.anydesk_password, now, now))
    return {"ok": True}


@app.get("/admin/anydesk/{username}")
async def get_anydesk(username: str, _: bool = Depends(verify_admin_key)):
    """Admin ki-jbed AnyDesk ID + password dyal user"""
    with get_db() as db:
        row = db.execute(
            "SELECT anydesk_id, anydesk_password FROM users WHERE username=?",
            (username,)
        ).fetchone()
        if not row:
            return {"anydesk_id": None, "anydesk_password": None}
        return {
            "anydesk_id": row["anydesk_id"],
            "anydesk_password": row["anydesk_password"],
        }
