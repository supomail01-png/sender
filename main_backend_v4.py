...
"""
ProjectSender Backend v4
خادم متقدم مع دعم إنشاء EXE مخصص والإعدادات المتعددة + حذف الصور
"""
...
app = FastAPI(title="ProjectSender Backend v4")
...
@app.delete("/api/screenshots/{client_id}")
async def delete_all_screenshots(client_id: str, key: str = Depends(verify_admin_key)):
    """حذف جميع صور العميل"""
    try:
        client_dir = os.path.join(SCREENSHOTS_DIR, client_id)
        ...
        if os.path.exists(client_dir):
            ...
            if client_id in clients:
                clients[client_id]["screenshots_count"] = 0
                save_clients(clients)
        
        return {"status": "ok", "message": "All screenshots deleted"}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))

@app.delete("/api/screenshots/{client_id}/{filename}")
async def delete_screenshot(client_id: str, filename: str, key: str = Depends(verify_admin_key)):
    """حذف صورة واحدة"""
    try:
        client_dir = os.path.join(SCREENSHOTS_DIR, client_id)
        file_path = os.path.join(client_dir, filename)
        ...
        if os.path.exists(file_path):
            ...
            if client_id in clients:
                if os.path.exists(client_dir):
                    clients[client_id]["screenshots_count"] = len(os.listdir(client_dir))
                else:
                    clients[client_id]["screenshots_count"] = 0
                save_clients(clients)
